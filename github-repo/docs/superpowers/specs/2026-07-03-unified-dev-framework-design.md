...
| Agent 3 (开发) | 技术预研+POC、架构设计、详细设计、Specs 编写、TDD 构建 | `test-driven-development`, `webapp-testing` |
| Agent 4 (质量) | 设计评审、代码评审+合规检查、安全扫描、端到端验证+Bug修复 | `code-quality-controller`, `frontend-design`, `code-review`, `TRAE-security-review`, `bug-fixer` |
...
| E2E 验收场景 | proposal.md 内嵌章节 | 场景名称 / 用户故事 / 前置条件 / 操作步骤 / 预期结果 / 优先级（P0/P1/P2）
...
| `specs/{capability}/contract.md` | 从 spec 推导的三层契约（API 级/模块级/函数级的前置/后置/不变式）|
| `tasks.md` | `project-planning-expert` 编排的开发任务清单 |

**节奏**: 分层审批（architecture → detailed-design → specs），capability 可并行
...
2. **读 contract.md → 翻译契约到测试** — 将前置/后置/不变式翻译为可执行测试（TDD RED）
3. **写测试** — pytest 单元测试 + 集成测试（Spec → TDD RED）
4. **写实现代码**（含运行时断言 require/ensure/invariant_check）
5. **跑测试** — 单元测试 + 集成测试 + 契约测试
6. **E2E 测试** — 同步编写 Playwright E2E 测试（TDD 外循环，has_ui: true 时）
7. **功能测试** — `webapp-testing`（调试验证）
8. **代码评审** — `code-review`（含 spec 合规检查 + 契约合规检查）
9. **安全扫描** — `TRAE-security-review`
10. **通过 → git commit**
...
**产出物**：
- `e2e/tests/*.spec.js` — E2E 测试用例（@P0 / @P1 tag）
- `e2e/fixtures/seed.py` — 测试数据生成
- `e2e/helpers/pages.js` — Page Object 封装

**工具**: `test-driven-development`、`webapp-testing`、`code-review`、`TRAE-security-review`
...
**目标**: E2E 自动化回归 + 人工探索性测试，发现并修复所有 Bug，补充测试缺口。

```
1. 执行全量 E2E 回归（Playwright，has_ui: true 时）
   ├── P0 场景全部通过 → 继续
...
**工具**: `bug-fixer`（含合并的 TRAE-debugger 运行时调试能力）、`webapp-testing`（E2E 回归执行）

**E2E 门禁** (Agent 4 执行，has_ui: true 时):
...
│    └── 流程短板：哪个阶段门禁没有挡住问题？              │
│                                                  │
│ 3. E2E 测试指标分析（has_ui: true 时）               │
│    ├── P0 覆盖率：是否 100%？                       │
│    ├── BUG 拦截率：E2E 拦截 BUG 占比是否 ≥80%？     │
│    ├── 回归执行时间：是否 ≤10 分钟？                 │
│    └── 未达标项产出根因分析与改进措施                  │
│                                                  │
│ 4. 契约违规分析                                     │
...
| Skill | 变更内容 |
|-------|---------|
| `bug-fixer` | + 原则 8「测试反哺原则」; 合并 TRAE-debugger 运行时调试能力（作为路径 A/B 增强模式） |
| `code-review` | + spec 合规检查项 + 代码规约检查项 |
| `TRAE-debugger` | 合入 bug-fixer 后删除 |

---

## 6. 目录结构

> 详见 [项目目录结构规范](file:///Users/dylanren/Documents/trae_projects/merge-books/docs/superpowers/specs/2026-07-03-project-structure-spec.md)
