...
2. **写测试** — pytest 单元测试 + 集成测试 (TDD 内层循环)
3. **写实现代码**
4. **跑测试** — 单元测试 + 集成测试
5. **E2E 测试** — 同步编写 Playwright E2E 测试（TDD 外循环，has_ui: true 时）
6. **功能测试** — `webapp-testing`（调试验证）
7. **代码评审** — `code-review`（含 spec 合规检查 + 代码规约检查）
8. **安全扫描** — `TRAE-security-review`
9. **通过 → git commit**

**TDD 内外双循环**：
```
TDD 内循环：RED(单元测试) → GREEN(实现) → REFACTOR
TDD 外循环：功能实现 → E2E 测试编写 → E2E 测试通过（门禁）
```
