#!/usr/bin/env python3
"""Verify the V1-structured MASE v2.3 training PPTX."""
...
DEFAULT_SOURCE = ROOT / "training/mase-framework/mase-training-v2.3.yaml"
DEFAULT_DECK = ROOT / "training/mase-framework/MASE框架培训讲义V2.3.pptx"
...
def verify(source_path: Path = DEFAULT_SOURCE, deck_path: Path = DEFAULT_DECK) -> dict[str, Any]:
    ...
    if len(slides) != 37 or len(v1.slides) != 37 or len(prs.slides) != 37:
        errors.append(
            f"expected 37 source/template/output slides, got {len(slides)}/{len(v1.slides)}/{len(prs.slides)}"
        )
    if (prs.slide_width, prs.slide_height) != (v1.slide_width, v1.slide_height):
        errors.append("V2.3 canvas does not match the V1 template")
    ...
    logo_issues = 0
    new_out_of_bounds = 0
    for index, (spec, original, revised) in enumerate(zip(slides, v1.slides, prs.slides), 1):
        ...
        if page_number in original_text and page_number not in text:
            errors.append(f"slide {index} lost its V1 page number")
        for expected in expected_strings(spec):
            if expected not in text:
                errors.append(f"slide {index} missing source text: {expected[:32]}")

        if index != 4 and shape_geometry(revised) != shape_geometry(original):
            geometry_mismatches += 1
        if picture_geometry(revised) != picture_geometry(original) or len(picture_geometry(revised)) != 1:
            logo_issues += 1
        for original_shape, revised_shape in zip(original.shapes, revised.shapes):
            if inside_canvas(original_shape, v1) and not inside_canvas(revised_shape, prs):
                new_out_of_bounds += 1
        for revised_shape in list(revised.shapes)[len(original.shapes) :]:
    ...
    for explanation in ("能力局部升级", "冻结候选", "证据会过期", "例如"):
    ...
    if editable < 180:
        errors.append(f"expected at least 180 editable text shapes, got {editable}")
    ...
    for spec in slides:
        ...
        for update in updates:
            ...
            extended_line = (spec["number"], update["slot"]) in {(13, "section-note"), (20, "footer")}
