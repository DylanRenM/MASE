...
The V2.4 deck SHALL preserve the V1 37-slide teaching skeleton and SHALL inherit the V1 slide shapes, geometry, typography, colors, section rhythm, and the Measures logo at the top-right of every slide. The sequence SHALL remain concepts, four Agents, six process states, project/tool matrix, individual tools, quick start, and closing; current v2.4 rules SHALL be taught inside those familiar slots.

#### Scenario: Existing learner opens the revised deck
- **WHEN** a learner familiar with V1 moves through the V2.4 deck
- **THEN** the learner recognizes the same chapter transitions and page patterns while seeing corrected v2.4 content

#### Scenario: Visual structure is compared automatically
- **WHEN** the verifier compares V1 and V2.4 page by page
- **THEN** every slide has the same shape types and geometry, including the Measures logo picture at its original position and size, while all V2.4 text remains editable
